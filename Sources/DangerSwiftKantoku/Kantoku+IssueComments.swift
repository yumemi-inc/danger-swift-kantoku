...
extension Kantoku {
    ...
    func post(_ summaries: [PostableIssueSummary], as level: CommentLevel) {
        for summary in summaries {
            ...
            let filePath = summary.documentLocation?.url// summary.documentLocation?.relativePath(against: workingDirectoryPath)

            if let filePath = filePath {
                let lineNumber = 0// filePath.queries?.endingLineNumber
...
//                post(as: level)(message, filePath.filePath, lineNumber.map({ $0 + 1 }) ?? 0)
                post(as: level)(message, filePath, lineNumber)
            } else {
